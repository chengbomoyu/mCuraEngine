...
void CommandLine::sliceNext(){
    ...
    for (size_t argument_index = 2; argument_index < arguments.size(); argument_index++){//参数①-0 curaengine.exe
                                                                                         //参数②-1 slice
                                                                                         //参数③-2 后续的命令行参数
        std::string argument = arguments[argument_index];
        if (argument[0] == '-'){
            if (argument[1] == '-'){} //从 "--" 开始的指令，是进行额外的切片，暂时去除。
             else{
                ...
                switch(argument[1]) {
                    ...
                    case 'o': {
                        argument_index++;
                        argument = arguments[argument_index];
                        FffProcessor::getInstance()->setTargetFile(argument.c_str())
                        break;
                    }
                    case 'g': break;
                    case 's': break;
